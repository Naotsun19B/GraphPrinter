...
namespace GraphPrinter
{
	...
	public:
		// Constructor.
		TInnerPrinter(TPrintOptions* InPrintOptions, TRestoreOptions* InRestoreOptions)
			: PrintOptions(InPrintOptions)
			, RestoreOptions(InRestoreOptions)
		{
		}
		...
		virtual bool CanPrintWidget() const override
		{
			...
			const TSharedPtr<TWidget> FoundWidget = FindTargetWidget(PrintOptions->TargetWidget);
				...
				FGraphPrinterUtils::ShowNotification(
					...
					TArray<FNotificationInteraction>{
						FNotificationInteraction(
							...
							{
								FGraphPrinterUtils::OpenFolderWithExplorer(WidgetPrinterParams.Filename);
							})
						)
					}
				);
